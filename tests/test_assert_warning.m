...
function error_warning_not_thrown(warning_id)
    error(warning_id,'Warning ''%s'' not thrown',warning_id);

function error_if_wrong_id_thrown(expected_warning_id, thrown_warning_id)
    if ~strcmp(thrown_warning_id,...
                    expected_warning_id)
        error_id='moxunit:wrongWarningRaised';
        error(error_id,['Warning raised with id ''%s'', expected '...
                        'id ''%s'''],...
