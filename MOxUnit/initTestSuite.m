...
% NNO Jan 2014
    ...
    get_example_syntax=@()sprintf('%s\n',...
                            ...
                            ['        try % use of "localfunctions" '...
                                            'is necessary in Matlab '...
                                            '2016b and later'],...
                            ['            test_functions='...
                                        'localfunctions();'],...
...
%         name=func2str(handle);
%
%         if ~has_error && ~moxunit_util_regexp_matches(name,test_re)
%             % name indicates it is not a test, skip
%             continue;
%         end
%
%         idx=find(strcmp(name,sub_func_names));
%         if ~isempty(idx)
%             assert(numel(idx)==1, 'name match should be unique');
%             if sub_func_struct(idx).nargout==0
%                 test_case=MOxUnitFunctionHandleTestCase(...
%                                                     name,...
%                                                     caller_fn, handle);
%                 test_suite=addTest(test_suite, test_case);
